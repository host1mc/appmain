...
(function () {
  ...
  var PANEL_BASE = (function () {
    try {
      var meta = document.querySelector('meta[name="panel-base"]');
  ...
  var FPJS_URL = PANEL_BASE + '/static/v2.js';
  ...
  var SAME_ORIGIN_BAIT = PANEL_BASE + '/static/ads.js';
  var SAME_ORIGIN_CONTROL = PANEL_BASE + '/static/app.css';
  ...
  var BOUNCE_KEY = 'fp_guard_bounces_panel';
  var BOUNCE_LIMIT = 2;
  ...
  var FAILED_UNIT_KEY = 'fp_guard_failed_unit_panel';
  ...
  var LOOP_GAVEUP_KEY = 'fp_guard_gaveup_panel';
  ...
  var RELEASED_KEY = 'fp_guard_released_panel';
  ...
  var PANEL_HOME = PANEL_BASE + '/';

  function safeReturnPath(value) {
    if (typeof value !== 'string' || value.charAt(0) !== '/') return PANEL_HOME;
    if (value.charAt(1) === '/' || value.indexOf('\\') !== -1) return PANEL_HOME;
    if (/[\u0000-\u001f\u007f]/.test(value)) return PANEL_HOME;
    return value;
  }
  ...
  function bootBlockedPage() {
    ...
    function failedUnitBlocked() {
      var src;
      try { src = window.sessionStorage.getItem(FAILED_UNIT_KEY); } catch (e) { src = null; }
      if (!src) return Promise.resolve(false);
      return probeNet(src).then(function (r) { return r === false; }, function () { return false; });
    }
    function returnHome() {
      if (leaving) return;
      leaving = true;
      ...
      if (!back) {
        ...
        try { back = m ? decodeURIComponent(m[1]) : PANEL_HOME; }
        catch (e) { back = PANEL_HOME; }
      }
      window.location.replace(safeReturnPath(back));
    }
    ...
    function tick() {
      ...
      Promise.all([
        safeAsync(detectAdblock),
        safeAsync(failedUnitBlocked)
      ]).then(function (r) {
        if (r[0] !== true && r[1] !== true) { stopped = true; returnHome(); return; }
        if (r[0] !== true && r[1] === true) {
          ...
          unitOnlyPolls += 1;
          if (unitOnlyPolls > UNIT_ONLY_LIMIT) {
            debug('unit host still refusing, no blocker found - releasing');
            stopped = true;
            returnHome();
            return;
          }
