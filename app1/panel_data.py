...
_INSERT_USER = (
    "INSERT INTO panel_users (id, username, password_hash, created_at) "
    "VALUES (:id, :uname, :ph, :created)"
)
...
_INSERT_ACTIVITY = (
    "INSERT INTO panel_activity (user_id, server_id, action, detail, created_at) "
    "VALUES (:u_id, :sid, :action, :detail, :created)"
)

_SELECT_ACTIVITY_FOR_USER = (
    "SELECT a.id AS id, a.user_id AS user_id, a.server_id AS server_id, "
    "a.action AS action, a.detail AS detail, a.created_at AS created_at, "
    "u.username AS username "
    "FROM panel_activity a JOIN panel_users u ON u.id = a.user_id "
    "WHERE a.user_id=:u_id ORDER BY a.id DESC FETCH FIRST :lim ROWS ONLY"
)

_PANEL_TABLES = ("PANEL_USERS", "PANEL_SERVERS", "PANEL_ACTIVITY")
...
_CREATE_DDL = {
    ...
    "PANEL_ACTIVITY": (
        (
            ...
            "user_id VARCHAR2(36 CHAR) NOT NULL, "
            "server_id VARCHAR2(36 CHAR), "
            "action VARCHAR2(64 CHAR) NOT NULL, "
            "detail VARCHAR2(500 CHAR), "
            "created_at DATE NOT NULL, "
            "PRIMARY KEY (id), "
            "FOREIGN KEY(user_id) REFERENCES panel_users (id) ON DELETE CASCADE)",
            "created panel_activity",
        ),
        (
            "CREATE INDEX ix_panel_activity_user "
            "ON panel_activity (user_id, created_at)",
            "created ix_panel_activity_user",
        ),
    ),
}
...
def update_user_password(user_id, password_hash):
    ...
    try:
        cur = conn.cursor()
        cur.execute(_UPDATE_USER_PASSWORD, {"ph": stored, "id": key})
        conn.commit()
...
def log_activity(user_id, action, server_id=None, detail=None):
    from .store import _truncate_detail, _utc_now

    conn = sqlite3.connect(SQLITE_DB_PATH)
    try:
        detail = _truncate_detail(detail)
        conn.execute(
            "INSERT INTO activity (user_id, server_id, action, detail, created_at) "
            "VALUES (?, ?, ?, ?, ?)",
            (user_id, server_id, action, detail, _utc_now()),
        )
        conn.commit()
    finally:
        conn.close()
...
def list_activity(user_id, limit=200):
    return []
