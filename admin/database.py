...
def erase_user(uid):
    ...
    delete_user(), then remove the account row itself plus its fingerprint
    rows, so the account is gone for good and drops out of list_users().
    delete_user() stays row-preserving for the automated ban-appeal /
    inactivity purges, which need the identity + fingerprint to keep
    recognising a returning device — this path is the explicit "erase it"."""
    delete_user(uid)
    uconn = _user_conn()
    try:
        cur = uconn.cursor()
